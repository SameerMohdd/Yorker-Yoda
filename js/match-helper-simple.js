/**
 * Match Helper Functions for Yorker Yoda (Simple Version)
 * This file contains helper functions to fix issues with matches display
 * without adding any buttons or UI elements
 */
    ...
    try {
        ...
        for (const [key, match] of Object.entries(upcomingMatches)) {
            ...
            try {
                ...
                let team1Logo = '<img src="/api/placeholder/40/40" alt="Team Logo" />';
                let team2Logo = '<img src="/api/placeholder/40/40" alt="Team Logo" />';
        ...
        if (html === '') {
            upcomingMatchesList.innerHTML = `
                <div style="text-align: center; padding: 20px; color: #666;">
                    <p>No upcoming matches to display.</p>
                </div>
            `;
        } else {
...
function fixDataIssues() {
    console.log("Applying data fixes silently...");
...
function initMatchHelper() {
    console.log("Initializing match helper (simple version)...");
    
    // Check if upcoming matches are displayed
    const upcomingMatchesList = document.getElementById('upcoming-matches-list');
    const quickMatchButtons = document.getElementById('quick-match-buttons');
    
    // First, see if they're empty
    const upcomingEmpty = upcomingMatchesList && (!upcomingMatchesList.children || upcomingMatchesList.children.length === 0);
    const buttonsEmpty = quickMatchButtons && (!quickMatchButtons.children || quickMatchButtons.children.length === 0);
    
    if (upcomingEmpty || buttonsEmpty) {
        console.log("Empty match displays detected, applying fixes...");
